...
Normalized cutoff	0.0375 (300 / 8000)
These are 31-tap low-pass filter coefficients designed with a Hamming window (via Python scipy.signal.firwin, cutoff = 300 Hz):
Buffers 16 kHz input samples
Applies 31-tap FIR filter
Emits one filtered sample every 20 input samples → 800 Hz output

The accumulator is 32-bit, which is safe:
    Max input ~±32767
    Max coeff ~±528
    Max product per tap ≈ ±17M
    Total of 31 taps: ~±530M max (well within int32_t range)
The final >> 15 converts the Q15 result back to a standard integer.
This works great for audio-level or SDR signals, where I/Q samples are already in int16_t from an ADC or IQ converter.



 16kHz -> I/Q low pass 300Hz (31 coeff taps) -> 800Hz samples  (one 800Hz sample every 20 16kHz samples = 1.25ms ) 
 every 0, 15s, 30s, 45s: start buffer 
 buffer 128 samples 800Hz and inform FFT to run  (128 * (1/800) = 160ms = exact time of each symbol transmitted)
    FFT kiss_fft complex with I+jQ (not kiss_fftr)
    FFT results 64+64 slots of 6.25Hz
    show at the display waterfall (it will be just a line for each symbol, msg = 79 symbols on total)
    look at the 1-8 slot for signal to translate to FT8 symbol, convert to symbol (3bits), make a buffer
    after buffer complete, decode, convert to chars, show text to display
    if the msg is CQ, get the call sign, if it is an answer to your call, get the call sign

 buffer 128 samples 400Hz and inform FFT to run  (128 * (1/400) = 320ms)
    FFT NSAMP = FSAMP / FRES = 400 / 3.125 = 128
    FF NFREQ = FFT NSAMP / 2 = 128 / 2 = 64
    FFT results 64+64 slots of 3.125Hz
assim não da certo pq o sinal de 2 simbolos vão ficar misturados ja que cada um é transmitido por 160ms



setup for FT8
  edit msgs  %1=user call sign  %2=other call sign
  edit user call sign
  edit time clock on exact seconds (sinchronize the time clock with the received signal)
  signal report fixed or get from ???

start to use the Arduino Pro Mini DFlash? I2C commands to write/read NVM
when changing the MODE to FT8, it will open a msg area on the screen, for 5 msgs ? with scroll
use the rotary switch and enter to change the signal report or the line on screen?
when in FT8, try to receive, and show a the screen, if <enter> passes to answer/transmit engage the QSO
...
// Shared FIR coefficients (Q15 format)
const int16_t firCoeffs[TAPS] = {
   -6,  -17,  -30,  -40,  -46,  -40,  -17,   29,
   97,  181,  270,  349,  401,  408,  360,  259,
  117,  -40, -200, -346, -459, -522, -528, -470,
 -353, -191,   -7,  170,  314,  404,  423
};

// FIR state for I channel
int16_t firBufferI[TAPS] = {0};
int firIndexI = 0;

// FIR state for Q channel
int16_t firBufferQ[TAPS] = {0};
int firIndexQ = 0;

int downsampleCounter = 0;

void processIQSample(int16_t sampleI, int16_t sampleQ) {
  // --- Store new samples into circular buffers ---
  firBufferI[firIndexI] = sampleI;
  firBufferQ[firIndexQ] = sampleQ;
  
  firIndexI = (firIndexI + 1) % TAPS;
  firIndexQ = (firIndexQ + 1) % TAPS;

  // --- Downsample logic ---
  if (++downsampleCounter == DECIMATE_FACTOR) {
    downsampleCounter = 0;

    int32_t accI = 0;
    int32_t accQ = 0;

    int indexI = firIndexI;
    int indexQ = firIndexQ;

    // --- FIR filtering for I and Q ---
    for (int i = 0; i < TAPS; i++) {
      indexI = (indexI - 1 + TAPS) % TAPS;
      indexQ = (indexQ - 1 + TAPS) % TAPS;

      accI += (int32_t)firBufferI[indexI] * firCoeffs[i];
      accQ += (int32_t)firBufferQ[indexQ] * firCoeffs[i];
    }

    // --- Convert Q15 back to int16_t ---
    int16_t outputI = accI >> 15;
    int16_t outputQ = accQ >> 15;

    // --- Send to FT8 or IQ processing ---
    handleFT8IQSample(outputI, outputQ);
  }
}

void handleFT8IQSample(int16_t I, int16_t Q) {
  // Store/process filtered I/Q samples at 800 Hz
}
...
/*
For FT8
FT8 uses 8 tones, spaced 6.25 Hz apart (0 Hz to 50 Hz), so:
    You're interested in bins 1 to 8
    Bin 1 = 6.25 Hz
    Bin 8 = 50.00 Hz
Perfect match.
You just:
    Compute the FFT magnitude at each bin:
    magnitude=Re2+Im2
    magnitude=Re2+Im2
Or faster: power=Re2+Im2power=Re2+Im2
Look for the bin (1–8) with the highest energy to identify the current FT8 tone
*/
...
void computeFFT(int16_t I[], int16_t Q[]) {
  for (int i = 0; i < FFT_SIZE; i++) {
    in[i].r = I[i];  // Real part
    in[i].i = Q[i];  // Imag part
  }
  ...
  // Analyze bins 1 to 8
  for (int k = 1; k <= 8; k++) {
    float power = out[k].r * out[k].r + out[k].i * out[k].i;
    // Store or compare power to find strongest tone
  }
}


/*
 Typical FT8 QSO Flow (Simplified)
Time	Message	              Direction	  Purpose
T+0s	CQ N0CALL FN31	      You → World	You announce you’re calling
T+15s	K1ABC N0CALL FN42	    K1ABC → You	Someone answers you
T+30s	N0CALL K1ABC -10	    You → K1ABC	You send them a signal report
T+45s	K1ABC N0CALL R-11	    K1ABC → You	They acknowledge and reply
T+60s	N0CALL K1ABC 73	      You → K1ABC	You send 73 (QSO done)

75 bits total
    28 bits: Callsign 1
    28 bits: Callsign 2
    15–19 bits: Grid or signal report or “73”
    
How FT8 Automatic Replies Work
In a typical QSO:
    You call CQ
    → Software listens for replies
    You receive a message like K1ABC N0CALL FN42
    → Software automatically detects:
        It’s a response to your CQ
        The other station's call is K1ABC
        You are N0CALL
    Software auto-generates reply
    → N0CALL K1ABC -10 (SNR report)
    Auto-timed transmission
    → Sent exactly at the next 15-second slot (T+30s)
    If they respond with R-10, your software replies with 73, etc.    
