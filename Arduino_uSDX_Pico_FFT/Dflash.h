...
//FLASH_SECTOR_SIZE_     # 4KB  The size of one sector, in bytes (the minimum amount you can erase)
//FLASH_PAGE_SIZE       # 256B The size of one page, in bytes (the mimimum amount you can write)
//XIP_BASE              #     = 0x10000000 bytes for RAM
#define FLASH_SECTOR_SIZE_   4096
#define MAX_NPAGE     (FLASH_SECTOR_SIZE_ / FLASH_PAGE_SIZE)   //number of pages inside of a sector  4096 / 256 = 16 pages of 256 bytes on the sector
#define MAX_NBLOCK_IN_PAGE    (FLASH_PAGE_SIZE / DATA_BLOCK_SIZE)     //number of data blocks inside of a page
#define MAX_NBLOCK    (FLASH_SECTOR_SIZE_ / DATA_BLOCK_SIZE)     //4096 / 16 number of data blocks inside of a sector
#define FLASH_TARGET_OFFSET   (PICO_FLASH_SIZE_BYTES - FLASH_SECTOR_SIZE_)  // start address for the last sector on FLASH mem
...
#define DFLASH_ADDR_ERASE  FLASH_TARGET_OFFSET  // FLASH_SECTOR_SIZE_
